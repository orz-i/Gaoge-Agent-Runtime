...
it("addresses manual spawn, subtask cancellation and checkpoint approval inside the authorized Turn", async () => {
  ...
  expect(await client.harness.turns.createSubtask("turn/1", "researcher", "Inspect evidence")).toEqual(body);
  ...
  expect(fetcher.mock.calls.map(([url, init]) => [url, init.method, JSON.parse(init.body)])).toEqual([
    ["https://runtime.test/api/v1/harness/turns/turn%2F1/subtasks", "POST", { roleID: "researcher", goal: "Inspect evidence" }],

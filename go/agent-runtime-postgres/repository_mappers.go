...
func toRunModel(item *domain.Run) models.RunRecord {
	...
	return models.RunRecord{RunID: item.RunID, RequestID: item.RequestID, RuntimeKind: domain.NormalizeRuntimeKind(item.RuntimeKind), TenantID: item.Actor.TenantID, ActorID: item.Actor.ActorID, ThreadKind: item.Thread.Kind, ThreadID: item.Thread.ID, InputProjectionKind: item.InputProjection.Kind, InputProjectionID: item.InputProjection.ID, OutputProjectionKind: item.OutputProjection.Kind, OutputProjectionID: item.OutputProjection.ID, EnvironmentKind: item.Environment.Kind, EnvironmentID: item.Environment.ID, EnvironmentRevision: item.Environment.Revision, AgentManifestID: item.AgentManifest.ID, AgentManifestRevision: item.AgentManifest.Revision, AgentName: item.AgentName, WorkflowDefinitionID: item.WorkflowDefinition.ID, WorkflowDefinitionRevision: item.WorkflowDefinition.Revision, RootRunID: item.RootRunID, ParentRunID: item.ParentRunID, HandoffID: item.HandoffID, Depth: item.Depth, Goal: item.Goal, RunConfigSnapshotJSON: item.RunConfigSnapshotJSON, RequestFingerprint: item.RequestFingerprint, CurrentStepID: item.CurrentStepID, CurrentPlanID: item.CurrentPlanID, PendingInteractionID: item.PendingInteractionID, StatusReason: item.StatusReason, LastEventSeq: item.LastEventSeq, LastPresentationEventSeq: item.LastPresentationEventSeq, StateProjectionVersion: currentStateProjectionVersion, StartedBy: item.StartedBy, Endpoint: item.Endpoint, Provider: item.Provider, ProviderProtocol: item.ProviderProtocol, RequestedModelName: item.RequestedModelName, PlatformModelName: item.PlatformModelName, RoutedBindingCode: item.RoutedBindingCode, UpstreamName: item.UpstreamName, ModelVendor: item.ModelVendor, ModelIcon: item.ModelIcon, UpstreamModelName: item.UpstreamModelName, InputTokens: item.InputTokens, OutputTokens: item.OutputTokens, CacheReadTokens: item.CacheReadTokens, CacheWriteTokens: item.CacheWriteTokens, ReasoningTokens: item.ReasoningTokens, LLMCallsCount: item.LLMCallsCount, ToolCallsCount: item.ToolCallsCount, BilledCurrency: item.BilledCurrency, BilledNanousd: item.BilledNanousd, LastBillingSnapshotJSON: item.LastBillingSnapshotJSON, FirstTokenLatencyMS: item.FirstTokenLatencyMS, TotalLatencyMS: item.TotalLatencyMS, Status: item.Status, ErrorCode: item.ErrorCode, ErrorMessage: item.ErrorMessage, StartedAt: item.StartedAt, EndedAt: item.EndedAt}
}
...
func toStepModel(item *domain.Step) models.RunStep {
	...
	return models.RunStep{StepID: item.StepID, RunID: item.RunID, ParentStepID: item.ParentStepID, PlanID: item.PlanID, StepIndex: item.StepIndex, Attempt: item.Attempt, NodeID: item.NodeID, ActivationPath: item.ActivationPath, LaneID: item.LaneID, ActivationIndex: item.ActivationIndex, CompletionOrder: item.CompletionOrder, Kind: item.Kind, Title: item.Title, Description: item.Description, Status: item.Status, DependsOnJSON: item.DependsOnJSON, ExpectedToolsJSON: item.ExpectedToolsJSON, ResourceRefsJSON: item.ResourceRefsJSON, ApprovalRequired: item.ApprovalRequired, WaitingKind: item.WaitingKind, WaitingID: item.WaitingID, ChildRunID: item.ChildRunID, ResultSummary: item.ResultSummary, InputJSON: item.InputJSON, OutputJSON: item.OutputJSON, ErrorJSON: item.ErrorJSON, StartedAt: startedAt, EndedAt: item.EndedAt}
}
...
func toStepDomain(row models.RunStep) domain.Step {
	...
	return domain.Step{StepID: row.StepID, RunID: row.RunID, ParentStepID: row.ParentStepID, PlanID: row.PlanID, StepIndex: row.StepIndex, Attempt: row.Attempt, NodeID: row.NodeID, ActivationPath: row.ActivationPath, LaneID: row.LaneID, ActivationIndex: row.ActivationIndex, CompletionOrder: row.CompletionOrder, Kind: row.Kind, Title: row.Title, Description: row.Description, Status: row.Status, DependsOnJSON: row.DependsOnJSON, ExpectedToolsJSON: row.ExpectedToolsJSON, ResourceRefsJSON: row.ResourceRefsJSON, ApprovalRequired: row.ApprovalRequired, WaitingKind: row.WaitingKind, WaitingID: row.WaitingID, ChildRunID: row.ChildRunID, ResultSummary: row.ResultSummary, InputJSON: row.InputJSON, OutputJSON: row.OutputJSON, ErrorJSON: row.ErrorJSON, StartedAt: startedAt, EndedAt: row.EndedAt, CreatedAt: row.CreatedAt, UpdatedAt: row.UpdatedAt}
}

...
func (m *Module) RegisterRoutes(auth *gin.RouterGroup) {
	...
	routes.POST("/runs/:run_id/handoff-joins", m.Handler.CreateRunHandoffJoin)
	routes.GET("/runs/:run_id/handoff-joins", m.Handler.ListRunHandoffJoins)
	routes.GET("/runs/:run_id/handoff-joins/:join_id", m.Handler.GetRunHandoffJoin)

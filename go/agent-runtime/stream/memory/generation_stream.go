...
type generationStream struct {
	...
	seq             int64
	events          []agentruntime.GenerationStreamMessage
	notify          chan struct{}
}
...
func (c *Cache) GetGenerationStreamOwner(ctx context.Context, runID string) (domain.ActorRef, bool, error) {
	...
	now := time.Now()
	if stream == nil || stream.owner.ActorID == "" || stream.ownerExpired(now) {
		return domain.ActorRef{}, false, nil
	}
	return stream.owner, true, nil
}
...
func (c *Cache) AppendGenerationStreamEvent(ctx context.Context, runID string, payloadJSON string, maxEvents int64, ttl time.Duration) (agentruntime.GenerationStreamMessage, error) {
	...
	stream := c.ensureStreamLocked(runID)
	stream.seq++
	record := agentruntime.GenerationStreamMessage{
...
func (c *Cache) ListGenerationStreamEvents(ctx context.Context, runID string, limit int64) ([]agentruntime.GenerationStreamMessage, error) {
	...
	if limit <= 0 || int(limit) >= len(stream.events) {
		return append([]agentruntime.GenerationStreamMessage(nil), stream.events...), nil
	}
	return append([]agentruntime.GenerationStreamMessage(nil), stream.events[len(stream.events)-int(limit):]...), nil
}
...
func generationEventsAfter(events []agentruntime.GenerationStreamMessage, afterSeq int64, limit int64) []agentruntime.GenerationStreamMessage {

...
func (a *ContextAssembler) Assemble(historyMessages []Message) ([]Message, AssemblyTrace) {
	...
	for _, s := range sorted {
		...
		if s.Required {
			systemMsgs = append(systemMsgs, Message{Role: valueSystem47BCD682, Content: content})
	...
	// 最终消息：system 前缀 + 历史消息
	result := make([]Message, 0, len(systemMsgs)+len(historyMessages))

...
const (
	valueAssistant5BB5F9FD  = "assistant"
	valueChunkCount80E5D7F7 = "chunk_count"
	valueFileIdAD7CFD2D     = "file_id"
	valueImage55145542      = "image"
	valueKey56627D0D        = "key"
	valueUserF1C9F5E1       = "user"
)
...
func buildPromptContextStateSignature(stableAttachments []AttachmentInput, preferenceMemories []MemoryItem) string {
...
func promptContextFileState(attachments []AttachmentInput) []map[string]interface{} {
	...
	for _, item := range attachments {
		if normalizeAttachmentKind(item.Kind, item.MimeType) == valueImage55145542 {
			continue
		}
	...
	for _, item := range items {
		result = append(result, map[string]interface{}{
			valueFileIdAD7CFD2D:     strings.TrimSpace(item.FileID),
			...
			valueChunkCount80E5D7F7: item.ChunkCount,
...
func promptContextMemoryState(memories []MemoryItem) []map[string]interface{} {
	if len(memories) == 0 {
		return nil
	}
...
func buildPromptContextConfigSignature(cfg Config) string {
	payload := map[string]interface{}{
		"embedding_enabled":           cfg.Retrieval.EmbeddingEnabled,
		"embedding_output_dimensions": cfg.Retrieval.EmbeddingOutputDimensions,
		"embedding_normalize":         cfg.Retrieval.EmbeddingNormalize,
		"embedding_model_signature":   strings.TrimSpace(cfg.Retrieval.EmbeddingModelSignature),
		"rag_enabled":                 cfg.Retrieval.Enabled,
		"rag_model":                   strings.TrimSpace(cfg.Retrieval.Model),
...
func promptStatePrefixMessages(messages []Message) []Message {
	lastUserIndex := -1
	for index := len(messages) - 1; index >= 0; index-- {
		if messages[index].Role == valueUserF1C9F5E1 {
			lastUserIndex = index
			break
		}
	}
...
func appendAssistantStateMessage(messages []Message, assistantText string) []Message {
	result := cloneLLMMessages(messages)
	result = append(result, Message{Role: valueAssistant5BB5F9FD, Content: assistantText})
	return result
}
...
func buildNextStatefulPrefixMessages(messages []Message, currentUserContent string, assistantText string) []Message {
	lastUserIndex := -1
	for index := len(messages) - 1; index >= 0; index-- {
		if messages[index].Role == valueUserF1C9F5E1 {
			lastUserIndex = index
	...
	result = append(result, Message{Role: valueUserF1C9F5E1, Content: currentUserContent})
	result = append(result, Message{Role: valueAssistant5BB5F9FD, Content: assistantText})
	return result
}
...
func normalizedToolDefinitions(tools []ToolDefinition) []ToolDefinition {
	if len(tools) == 0 {
		return nil
	}
